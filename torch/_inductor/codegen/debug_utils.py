    ...
    def codegen_intermediate_tensor_value_save(
        ...
        for i, arg in enumerate(args_to_save):
            ...
            if V.graph.cpp_wrapper:
                ...
                cwd = os.getcwd()
                if not os.path.exists(cwd + "/tmp/jit_inductor/"):
                    log.info(
                        "Creating tmp/jit_inductor/ directory for saving inductor intermediate tensor values."
                    )
                    os.makedirs(cwd + "/tmp/jit_inductor/")
                # Save the model to the directory
                saved_path = (
                    cwd
                    + "/tmp/jit_inductor/"
                    + f"{launch_prefix}_{kernel_name}_{arg}.pt"
                )
                log.info(
                    f"Saved intermediate tensor {arg} for {kernel_name} to {saved_path}"
                )
